...
<body>
	<form method="post" action="/midtermPHP/Editebs.php?id=8">
		<input type="text" name="id" value="<?php echo $row['id'] ?>">
		<div class="input-group my-2">
			<input type="text" class="form-control" placeholder="Name of Item" value="<?php echo $nameproduct; ?>" id="productName">
		</div>
		<div class="input-group my-2">
			<input type="text" class="form-control" placeholder="Number" value="<?php echo $detialpro; ?>" id="productDetail">
		</div>
		<div class="input-group my-2">
			<input type="text" class="form-control" placeholder="Cost Price" value="<?php echo $pricepro; ?>" id="productPrice">
		</div>
	</form>
...
<script>
	$(document).ready(function() {
		...
		function saveimage() {
			...
			$.ajax({
				url: "./uploadimageproduct.php",
				method: "POST",
				data: {
					image: base64data,
					product_name: $('#productName').val(),
					d_product: $('#productDetail').val(),
					p_price: $('#productPrice').val()
				},
				success: function(data) {
					console.log(data);

					location.href = './insertProduct.php'
